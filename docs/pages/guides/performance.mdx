...
Use `nova reload` instead of `nova restart` for zero-downtime deployments. NovaPM restarts instances one at a time, waiting for each to become ready before proceeding:
...
```bash
nova metrics api --metric memory --period 24h
```
...
- [ ] Use `nova reload` for zero-downtime deploys in cluster mode
