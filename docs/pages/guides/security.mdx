...
```bash
nova-pm agent connect <controller-host>:9800 --token $NOVA_AUTH_TOKEN
```
...
nova-pm token generate --save

# Update all agents (rolling update)
nova-pm agent update-token --new-token $NEW_NOVA_AUTH_TOKEN

# Revoke the old token on the controller
nova-pm token revoke <old-token-id>
```
...
nova-pm tls generate --out ~/.novapm/certs/

# This creates:
#   ~/.novapm/certs/server.key
#   ~/.novapm/certs/server.crt
#   ~/.novapm/certs/ca.crt
```
...
```bash
nova-pm agent connect <controller-host>:9800 \
  --tls \
  --ca /etc/nova/certs/ca.crt
```
...
```bash
nova-pm env set my-app DATABASE_URL "postgresql://user:pass@host/db" --secret
nova-pm env set my-app API_KEY "sk-abc123" --secret
```
...
export default {
  apps: [{
    ...
    // Secrets set via `nova-pm env set` are automatically injected
  }]
}
```

<Callout type="warning">
Avoid passing secrets through the `env` block in configuration files. Use `nova-pm env set --secret` to keep sensitive values out of version control and encrypt them at rest.
</Callout>
...
The `NOVA_HOME` directory (default: `~/.novapm`) contains sensitive data including process configurations, encrypted secrets, and logs.
...
chmod 700 ~/.novapm
chmod 700 ~/.novapm/db
chmod 600 ~/.novapm/db/nova-pm.db
chmod 700 ~/.novapm/logs
chmod 700 ~/.novapm/certs
chmod 600 ~/.novapm/certs/*.key
```

**Verify permissions:**

```bash
nova-pm doctor --check permissions
```
...
- [ ] Secrets are stored with `nova-pm env set --secret`, not in config files
