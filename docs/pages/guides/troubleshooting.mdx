...
- The socket file at `~/.nova/nova.sock` is stale -- remove it and restart
...
If a process has restarted more than 15 times in quick succession, NovaPM will mark it as `errored` and stop attempting restarts. Use `nova restart <app-name>` to reset the counter.
</Callout>
...
```bash
nova scale my-app 4  # Match to your core count
```

<Callout type="info">
Use `nova metrics <app-name>` to view historical CPU and memory trends and identify when spikes occur.
</Callout>
...
**Symptoms:** `nova status` returns garbled data or errors, processes show incorrect state.
...
cp ~/.nova/db/nova.db ~/.nova/db/nova.db.backup
...
|---|---|---|
| `ECONNREFUSED 127.0.0.1:9876` | Daemon not running | Run `nova daemon start` |
...
| `EACCES /home/user/.nova` | Permission denied | Fix ownership with `chown` |
| `Script not found` | Invalid script path in config | Verify the `script` path is correct and the file exists |
| `Maximum restart count reached` | Too many rapid restarts | Fix the root cause, then `nova restart <app>` |
...
1. Run `nova report` to generate a diagnostic report
2. Check the [GitHub Issues](https://github.com/nova-pm/nova/issues) for known problems
3. Include daemon logs (`~/.nova/logs/daemon.log`) when filing bug reports
